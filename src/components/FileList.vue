<template>
    <table>
        ...
        <tbody>
            <tr v-if="path !== ''"  @dblclick="changePath('..')" @click="isMobile ? changePath('..') : null" style="cursor: pointer;">
                <td v-if="!isMobile">
            ...
            <tr v-for="file in sortedFiles" :key="file.id" @click="isMobile && file.isDir ? changePath(file.url) : null" @dblclick="file.isDir ? changePath(file.url) : null" :style="[file.isDir ? {'cursor':'pointer'} : {'cursor':''}]">
                <td v-if="!isMobile">
                <Suspense>
                ...
                <td>
                    <div>
                        <svg @click="deleteFile(file)" class="delete" v-if="is_admin" viewBox="0 0 24 24"><path d="M6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7H6v12zM8 9h8v10H8V9zm7.5-5-1-1h-5l-1 1H5v2h14V4z"></path></svg>
...
<script setup>
    ...
    function deleteFile(file){
        if (confirm(`Voulez-vous vraiment supprimer ${file.name} ?`)) {
            ...
            axios.post(`/api/delete${file.url}`, formData)
            .then(() => {
